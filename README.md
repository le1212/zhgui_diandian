...
### 任务管理
- 多任务管理：新建、复制、重命名、切换
- 定时任务：一次性 / 每天 / 固定间隔三种触发，到期自动执行，错过与运行中冲突自动处理，双击计划可随时修改
- 后台运行：有启用中的定时任务时关闭主窗口会最小化到托盘，任务继续执行，托盘菜单可随时唤回或退出，定时触发时托盘气泡提醒
...
### 用户体验
- 莫兰迪灰绿配色，护眼舒适；深色 / 浅色主题头部一键切换，偏好自动记忆
- 任务删除即时可撤销：删除后 5 秒内 Toast 一键撤销，回收站兜底
- 键盘可达：Tab 遍历控件、回车 / 空格激活按钮、下拉方向键选择，焦点描边清晰
- 右键菜单单实例不叠加，失焦 / 点外自动收起，按所在显示器工作区定位
- 状态栏悬停查看最近状态历史与定时触发结果
- 窗口位置与尺寸记忆，多显示器下安全恢复不出屏
- 首次启动新手引导，三步上手
- 空状态引导：列表为空、计划为空等场景均有引导文案
- Toast 轻提示不叠加，危险操作使用自绘确认框（红色警示、回车默认取消）
- 数字输入越界自动纠正并闪红提示，超长列表显示细滚动条
...
├── theme.py                 # 主题层（浅色 / 深色调色板，启动时读定，切换重启生效）
